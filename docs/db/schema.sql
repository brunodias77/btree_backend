...
BEGIN
    ...
    random_part := UPPER(SUBSTRING(MD5(gen_random_uuid()::TEXT) FROM 1 FOR 9));
...
CREATE TABLE users.user_tokens (
                                   ...
                                   token_type VARCHAR(50)        NOT NULL
                                       CONSTRAINT chk_user_tokens_token_type
                                           CHECK (token_type IN (
                                                                 'EMAIL_VERIFICATION', 'PASSWORD_RESET', 'TWO_FACTOR',
                                                                 'TWO_FACTOR_SETUP', 'MAGIC_LINK', 'ACCOUNT_UNLOCK',
                                                                 'PHONE_VERIFICATION'
                                               )),
...
CREATE TABLE users.login_history_default PARTITION OF users.login_history DEFAULT;
...
CREATE INDEX idx_products_sku_trgm              ON catalog.products USING GIN (lower(sku) gin_trgm_ops)
    WHERE deleted_at IS NULL;
CREATE INDEX idx_products_short_description_trgm ON catalog.products USING GIN (lower(short_description) gin_trgm_ops)
    WHERE deleted_at IS NULL;
...
-- (V014 + V017) Required so Hibernate can bind string literals
-- against custom PostgreSQL enum columns.
-- ============================================================
...
-- Orders
CREATE CAST (character varying AS shared.order_status)        WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.cancellation_reason) WITH INOUT AS IMPLICIT;

-- Payments
CREATE CAST (character varying AS shared.payment_status)      WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.payment_method_type) WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.transaction_type)    WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.refund_status)       WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.chargeback_status)   WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.card_brand)          WITH INOUT AS IMPLICIT;

-- Coupons
CREATE CAST (character varying AS shared.coupon_type)         WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.coupon_scope)        WITH INOUT AS IMPLICIT;
CREATE CAST (character varying AS shared.coupon_status)       WITH INOUT AS IMPLICIT;

-- Users
CREATE CAST (character varying AS shared.token_type)          WITH INOUT AS IMPLICIT;
